...
const MyWithdrawalHistory = () => {
  ...
  useEffect(() => {
    const fetchWithdrawals = async () => {
      try {
        const res = await axios.get(`${API_BASE_URL}/api/transaction/withdrawals/${userId}`);
  ...
  const paginated = withdrawals.slice((page - 1) * itemsPerPage, page * itemsPerPage);
  ...
  return (
    <div className="w-screen flex justify-center h-screen bg-[#9195a3]">
      ...
      <div className="w-full max-w-[500px] mx-auto px-4 pb-24 font-poppins pt-[60px] bg-white">
        <h2 className="text-sm font-medium mb-4 text-start text-black">My Withdrawal History</h2>
